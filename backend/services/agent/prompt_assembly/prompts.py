# CALLING SPEC:
# - Purpose: render hosted and external-agent system prompts from Jinja templates.
# - Inputs: SystemPromptContext for hosted runs; none for external-agent instruction shell.
# - Outputs: rendered prompt strings with account context, memory, entity categories, and bh cheat sheet.
# - Side effects: none; templates load from the parent agent package directory.
...
from backend.cli_reference import render_bh_cheat_sheet, render_hosted_agent_bh_cheat_sheet

DEFAULT_USER_TIMEZONE = "America/Toronto"
_AGENT_TEMPLATE_DIR = Path(__file__).resolve().parent.parent
