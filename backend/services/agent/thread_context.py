# CALLING SPEC:
# - Purpose: build new-turn harness transcript from canonical transcript rows.
# - Inputs: SQLAlchemy session, thread id, user input, attachments, model/surface context.
# - Outputs: initial_transcript for RunRequest; reuses the thread's first hosted system row.
# - Side effects: reads transcript rows and assembles multimodal user content.
...
from backend.services.agent.message_history_prefixes import build_review_results_prefix_for_thread
