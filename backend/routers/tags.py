# CALLING SPEC:
# - Purpose: translate HTTP requests and responses for `tags` routes.
# - Inputs: callers that import `backend/routers/tags.py` and pass module-defined arguments or framework events.
# - Outputs: router callables and request/response adapters for `tags`.
...
from backend.services.access_scope import load_tag_for_principal
...
def update_tag(
    ...
    tag = load_tag_for_principal(db, tag_id=tag_id, principal=principal)
    update_tag_service(
        db,
        tag=tag,
