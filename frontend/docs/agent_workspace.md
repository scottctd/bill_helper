...
- persisted assistant reasoning collapses per committed harness step; live `reasoning_delta` updates feed the same expandable activity row until the durable assistant/step projection arrives
...
- compact tool-call snapshots are hydrated on demand from `GET /agent/tool-calls/{tool_call_id}`; hydration merges into the same-tab stream store and the cached thread detail so both unattached live stream rows and reopened turns reconcile ledger rows with hydrated snapshots
...
- user bubbles show only the typed message by default; when the backend exposes a differing `raw_prompt_markdown`, a collapsed **Raw prompt** disclosure renders the full model-facing text in a muted monospace block
