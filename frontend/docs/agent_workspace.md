...
- compact tool-call snapshots are hydrated on demand from `GET /agent/tool-calls/{tool_call_id}`; hydration merges into the same-tab stream store and the cached thread detail so reopened turns reconcile ledger rows and persisted compact snapshots
