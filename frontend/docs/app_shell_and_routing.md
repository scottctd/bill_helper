...
- collapsible left sidebar (`Sidebar.tsx`) with navigation links for `Dashboard`, `Agent`, `Import`, `Accounts`, `Entries`, `Groups`, `Entities`, `Properties`, and `Settings`
...
- `/filters` -> redirect to `/groups` (legacy deep links only)
- `/entries` -> entry list
