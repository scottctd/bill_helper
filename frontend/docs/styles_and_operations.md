...
- `agent/review/diff/*`
- `review/ReviewPanel.tsx` (shared review shell), `review/ReviewCardHeader.tsx`, `review/cardMetadata.ts`, `review/ReviewItemCard.tsx`, `review/ReviewFieldList.tsx`, `review/proposalFields.ts`, `review/ReviewToc.tsx`, `review/tocTree.ts`, `review/entryTocFields.ts`, `review/helpers.ts`, `review/mapImportProposal.ts`, `review/mapThreadReviewItem.ts`
...
- `Approve All` and `Reject All` use one batch review request per thread (or run for Telegram) instead of sequential per-item calls
