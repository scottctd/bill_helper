...
COPY docker/agent-workspace-requirements.txt /opt/bill-helper/docker/
RUN pip install --no-cache-dir -r /opt/bill-helper/docker/agent-workspace-requirements.txt

COPY pyproject.toml README.md /opt/bill-helper/
COPY backend /opt/bill-helper/backend
COPY telegram /opt/bill-helper/telegram
RUN pip install --no-cache-dir --no-deps /opt/bill-helper

RUN useradd --create-home --shell /bin/bash app \
    && mkdir -p /workspace \
    && chown -R app:app /workspace /home/app /opt/code-server-preinstalled-vsix
