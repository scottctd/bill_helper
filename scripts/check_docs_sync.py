# CALLING SPEC:
# - Purpose: run the `check_docs_sync` repository script.
# - Inputs: callers that import `scripts/check_docs_sync.py` and pass module-defined arguments or framework events.
# - Outputs: CLI-side workflow helpers and the `check_docs_sync` entrypoint.
...
def assert_system_prompt_example_is_generated_snapshot(errors: list[str]) -> None:
    path = DOCS_DIR / "features" / "system_prompt_example.md"
    if not path.exists():
        errors.append(f"Missing generated snapshot doc: {path.relative_to(ROOT)}")
        return
    contents = read_text(path)
    required_phrases = (
    ...
    for phrase in required_phrases:
        if phrase not in contents:
            errors.append(
                ...
                "(regenerate after editing hosted/external prompt templates, prompt includes, or `backend/cli_reference/`)."
            )
...
def main() -> int:
    ...
    assert_system_prompt_example_is_generated_snapshot(errors)
