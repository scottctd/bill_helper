...
class TelegramCommandRouter:
    ...
    def handle_threads(self, chat_id: int) -> str:
        ...
        if len(threads) > len(visible_threads):
            lines.append(f"…and {len(threads) - len(visible_threads)} more. Use /use <thread-id> for older threads.")
        lines.append("Use /use <number|thread-id> to switch the active thread.")
        return "\n".join(lines)

    def handle_use(self, chat_id: int, selector: str | None) -> str:
        normalized_selector = (selector or "").strip()
        if not normalized_selector:
            return "Usage: /use <number|thread-id>"
        ...
        if normalized_selector.isdigit():
            ...
            detail = self._api_client.get_thread(normalized_selector)
    ...
    def _help_text(self, *, intro: str | None = None) -> str:
        ...
        lines.extend(
            [
                ...
                "/use <number|thread-id> — switch the active thread",
...
def build_handlers_from_settings(
    ...
    *,
    ...
    return TelegramBotHandlers(command_router=router, message_handler=content_handler)
