...
**Do not edit this file by hand.** It is overwritten by `scripts/render_agent_system_prompt_snapshot.py`. Regenerate with `uv run python scripts/render_agent_system_prompt_snapshot.py`. Canonical prompt sources: `backend/services/agent/system_prompt.j2`, `backend/services/agent/prompt_includes/`, and `backend/services/agent/prompts.py`; the embedded `bh` cheat sheet comes from `backend/cli/reference.py`.
