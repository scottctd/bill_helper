...
| --------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Runtime         | `backend/services/agent/runtime.py`, `backend/services/agent/runtime_loop.py`                                                                                                                                                                                        | Stable runtime entrypoints plus dedicated run-loop adapters for bounded tool execution, tool turn persistence, and terminal assistant completion                                                                                                                                                                                                                                                                                                                             |
| Model client    | `backend/services/agent/model_client.py`, `backend/services/agent/model_client_support/`                                                                                                                                                                                                                                                    | Thin public LiteLLM client seam backed by grouped support modules for provider/env validation, prompt-cache breakpoint injection, retry-enabled completion calls, stream delta reconciliation, and usage normalization                                                                                                                                                                                                                                                                                                               |
...
| Tools           | `backend/services/agent/tool_args/`, `backend/services/agent/change_contracts/`, `backend/services/agent/read_tools/`, `backend/services/agent/session_tools/`, `backend/services/agent/proposals/`, `backend/services/agent/entry_references.py`, `backend/services/agent/group_references.py`, `backend/services/agent/proposal_metadata.py`, `backend/services/agent/proposal_patching.py`, `backend/services/agent/tool_runtime.py`, `backend/services/agent/tool_runtime_support/`, `backend/services/agent/tool_results.py`, `backend/services/agent/tools.py`, `backend/services/agent/threads.py` | Split tool contract/runtime stack: focused tool-input modules for read/session/admin commands, proposal payload contracts grouped by catalog/entry/group family under `change_contracts/`, proposal-family handlers under `proposals/` (catalog, entries, groups, membership, normalization, pending admin), lookup-only handlers under `read_tools/`, session-scoped non-proposal handlers under `session_tools/`, shared tool result formatting, thread-title validation/rename helpers, shared entry/group lookup helpers, proposal metadata mapping, patch-map helpers, a thin public tool-runtime seam with grouped registry/schema/execution support modules, and a thin facade |
| Review/apply    | `backend/services/agent/reviews/`, `backend/services/agent/apply/`                                                                                                                                                                                                   | Approval/rejection workflow plus apply handlers for proposed CRUD changes                                                                                                                                                                                                                                                                                                                                                                                                      |
| API router      | `backend/routers/agent.py`, `backend/routers/agent_threads.py`, `backend/routers/agent_runs.py`, `backend/routers/agent_reviews.py`, `backend/routers/agent_attachments.py`                                                                                                                                                                                                                 | Thin transport aggregator plus focused HTTP modules for threads, runs, review actions, and attachment downloads; the thread routes own their small SSE/upload helper boundary directly                                                                                                                                                                                                                                                                                         |
...
Tool execution is composed from `backend/services/agent/tool_runtime.py` (thin public seam) and `backend/services/agent/tool_runtime_support/` (`definitions.py`, `schema.py`, family-owned `catalog_read.py` / `catalog_session.py` / `catalog_proposals.py`, merged `catalog.py`, and `execution.py`) with handlers in `backend/services/agent/read_tools/` (`catalog.py`, `entries.py`, `groups.py`, `proposals.py`, plus shared `common.py`), `backend/services/agent/session_tools/` (`progress.py`, `memory.py`, `threads.py`), the proposal-family modules under `backend/services/agent/proposals/` (`catalog.py`, `entries.py`, `groups.py`, the `group_memberships/` package for membership canonicalization, validation, and handlers, family-owned normalization modules plus a small `normalization.py` registry, `pending.py`, and shared `common.py`), shared entry/group reference helpers in `backend/services/agent/entry_references.py` and `backend/services/agent/group_references.py`, proposal-domain metadata in `backend/services/agent/proposal_metadata.py`, focused non-proposal tool inputs in `backend/services/agent/tool_args/`, proposal contract families in `backend/services/agent/change_contracts/` (`catalog.py`, `entries.py`, `groups.py`, plus shared `common.py` and registry/patch helpers in `__init__.py` + `patches.py`), thread-title helpers in `backend/services/agent/threads.py`, patch-map helpers in `backend/services/agent/proposal_patching.py`, and shared result formatting in `backend/services/agent/tool_results.py`. `backend/services/agent/tools.py` is a thin facade that re-exports runtime interfaces. Each tool returns plain-text `content` to the model.
...
| File                                              | Purpose                                                              |
| ------------------------------------------------- | -------------------------------------------------------------------- |
| `backend/services/agent/tool_args/`               | Focused tool-input modules for read/session/admin commands           |
| `backend/services/agent/change_contracts/`        | Proposal payload contracts split by catalog, entry, and group domains, with shared normalization and patch validation helpers |
| `backend/services/agent/read_tools/`              | Read/progress tool handlers grouped by lookup domain                 |
| `backend/services/agent/session_tools/`          | Session-scoped non-proposal handlers for progress, memory, and thread rename |
| `backend/services/agent/proposals/` | Proposal-family handlers, payload normalization, and pending-proposal coordination |
...
| `backend/services/agent/tool_runtime.py`, `backend/services/agent/tool_runtime_support/` | Thin public tool-runtime seam plus grouped registry/schema/execution policy |
| `backend/services/agent/tools.py`                 | Thin tool facade/re-export layer                                     |
| `backend/services/agent/runtime.py`               | Runtime entrypoints and stable model/count seams                     |
| `backend/services/agent/runtime_loop.py`          | Run-loop adapters and tool-turn orchestration                        |
| `backend/services/agent/model_client.py`, `backend/services/agent/model_client_support/` | LiteLLM client seam plus grouped retry/stream/env helpers            |
...
| `backend/services/agent/reviews/`                 | Review workflow, dependency checks, and override normalization       |
| `backend/services/agent/apply/`                   | Apply handlers for approved changes                                  |
| `backend/routers/agent.py`                        | Agent router aggregator                                              |
| `backend/routers/agent_threads.py`                | Thread list/detail/send/stream endpoints                             |
| `backend/routers/agent_runs.py`                   | Run detail/tool-call/interrupt endpoints                             |
| `backend/routers/agent_reviews.py`                | Review action endpoints                                              |
| `backend/routers/agent_attachments.py`            | Attachment download endpoint                                         |
